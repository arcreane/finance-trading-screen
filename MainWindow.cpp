...
void MainWindow::setupUi() {
    ...
    TickerPlaceholder *tickerWidget = new TickerPlaceholder(this);
    // On peut fixer une hauteur pour qu'il ressemble à une barre et ne prenne pas trop de place
    // tickerWidget->setFixedHeight(60); // Décommentez si vous voulez une hauteur fixe
    ...
    // changez QVBoxLayout en QHBoxLayout ici :
    QHBoxLayout* pinkLayout = new QHBoxLayout(pinkContainer);
    pinkLayout->setContentsMargins(0,0,0,0); // Bonnes pratiques pour éviter les marges doubles
    ...
    zone3->setStyleSheet("background-color: #1e1e1e; border: 1px solid #ff00ff;");
    ...
    zone5->setStyleSheet("background-color: #1e1e1e; border: 1px solid #ffff00;");
    ...
    mainLayout->addWidget(yellowContainer, 1); // Yellow takes remaining width (stretch 1)
